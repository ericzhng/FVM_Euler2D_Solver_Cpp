...
namespace fvm2d
{
    ...
    PartitionMesh load_partition_for_rank(const std::string &mesh_dir, int rank)
    {
        ...
        // Auto-detect format: check for JSON partition files (new format) first
        std::string json_path = mesh_dir + "/partition_" + std::to_string(rank) + ".json";
        if (fs::exists(json_path))
        {
            // New format: VTU + JSON + boundaries
            return load_partition_mesh_from_dir(mesh_dir, rank);
        }

        // Fallback to old .mesh format
